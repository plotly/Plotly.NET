...
type Layout() =
    ...
    let mutable _xaxis: Axis option = None
    let mutable _xaxis1: Axis option = None
    let mutable _xaxis2: Axis option = None
    let mutable _yaxis: Axis option = None
    let mutable _yaxis2: Axis option = None
    // TODO: Scene
    ...
    let mutable _radialaxis: Axis option = None
    let mutable _angularaxis: Axis option = None
...
//        and set value = _scene <- Some value
    ...
    member __.fontOption
        with get () = _font
        and set value = _font <- value

    [<Newtonsoft.Json.JsonIgnore>]
    member __.titleOption
        with get () = _title
        and set value = _title <- value

    [<Newtonsoft.Json.JsonIgnore>]
    member __.titlefontOption
        with get () = _titlefont
        and set value = _titlefont <- value

    [<Newtonsoft.Json.JsonIgnore>]
    member __.autosizeOption
        with get () = _autosize
        and set value = _autosize <- value

    [<Newtonsoft.Json.JsonIgnore>]
    member __.widthOption
        with get () = _width
        and set value = _width <- value

    [<Newtonsoft.Json.JsonIgnore>]
    member __.heightOption
        with get () = _height
        and set value = _height <- value

    [<Newtonsoft.Json.JsonIgnore>]
    member __.marginOption
        with get () = _margin
        and set value = _margin <- value

    [<Newtonsoft.Json.JsonIgnore>]
    member __.paper_bgcolorOption
        with get () = _paper_bgcolor
        and set value = _paper_bgcolor <- value

    [<Newtonsoft.Json.JsonIgnore>]
    member __.plot_bgcolorOption
        with get () = _plot_bgcolor
        and set value = _plot_bgcolor <- value

    [<Newtonsoft.Json.JsonIgnore>]
    member __.separatorsOption
        with get () = _separators
        and set value = _separators <- value

    [<Newtonsoft.Json.JsonIgnore>]
    member __.hidesourcesOption
        with get () = _hidesources
        and set value = _hidesources <- value

    [<Newtonsoft.Json.JsonIgnore>]
    member __.smithOption
        with get () = _smith
        and set value = _smith <-  value

    [<Newtonsoft.Json.JsonIgnore>]
    member __.showlegendOption
        with get () = _showlegend
        and set value = _showlegend <-  value

    [<Newtonsoft.Json.JsonIgnore>]
    member __.dragmodeOption
        with get () = _dragmode
        and set value = _dragmode <-  value

    [<Newtonsoft.Json.JsonIgnore>]
    member __.hovermodeOption
        with get () = _hovermode
        and set value = _hovermode <-  value
        
    [<Newtonsoft.Json.JsonIgnore>]
    member __.xaxisOption
        with get () = _xaxis
        and set value = _xaxis <-  value

    [<Newtonsoft.Json.JsonIgnore>]
    member __.xaxis1Option
        with get () = _xaxis1
        and set value = _xaxis1 <-  value
        
    [<Newtonsoft.Json.JsonIgnore>]
    member __.xaxis2Option
        with get () = _xaxis2
        and set value = _xaxis2 <-  value
        
    [<Newtonsoft.Json.JsonIgnore>]
    member __.yaxisOption
        with get () = _yaxis
        and set value = _yaxis <-  value
        
    [<Newtonsoft.Json.JsonIgnore>]
    member __.yaxis2Option
        with get () = _yaxis2
        and set value = _yaxis <-  value
        
//    [<Newtonsoft.Json.JsonIgnore>]
//    member __.sceneOption
//        with get () = _scene
//        and set value = _scene <-  value
        
    [<Newtonsoft.Json.JsonIgnore>]
    member __.geoOption
        with get () = _geo
        and set value = _geo <-  value

    [<Newtonsoft.Json.JsonIgnore>]
    member __.legendOption
        with get () = _legend
        and set value = _legend <-  value

    [<Newtonsoft.Json.JsonIgnore>]
    member __.annotationsOption
        with get () = _annotations
        and set value = _annotations <-  value

    [<Newtonsoft.Json.JsonIgnore>]
    member __.shapesOption
        with get () = _shapes
        and set value = _shapes <-  value

    [<Newtonsoft.Json.JsonIgnore>]
    member __.radialaxisOption
        with get () = _radialaxis
        and set value = _radialaxis <-  value

    [<Newtonsoft.Json.JsonIgnore>]
    member __.angularaxisOption
        with get () = _angularaxis
        and set value = _angularaxis <-  value

    [<Newtonsoft.Json.JsonIgnore>]
    member __.directionOption
        with get () = _direction
        and set value = _direction <-  value

    [<Newtonsoft.Json.JsonIgnore>]
    member __.orientationOption
        with get () = _orientation
        and set value = _orientation <-  value

    [<Newtonsoft.Json.JsonIgnore>]
    member __.barmodeOption
        with get () = _barmode
        and set value = _barmode <-  value

    [<Newtonsoft.Json.JsonIgnore>]
    member __.bargapOption
        with get () = _bargap
        and set value = _bargap <-  value
