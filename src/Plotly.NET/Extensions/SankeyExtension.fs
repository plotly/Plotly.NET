...
module SankeyExtension =
    ...
    type TraceDomainStyle with
        ...
        static member Sankey
            ...
            (fun (trace: ('T :> Trace)) ->
                let nonUniqueLabels =
                    nodes |> Seq.countBy (fun x -> x.Label) |> Seq.filter (fun (_, c) -> c > 1)

                if nonUniqueLabels |> Seq.length > 0 then
                    failwithf "duplicated label names %A" (nonUniqueLabels |> Seq.map fst)

                let lblMap =
                    nodes |> Seq.mapi (fun i x -> x.Label, i) |> Map.ofSeq // give each node an index
                ...
                let node =
                    let groups =
                        nodes
                        |> Seq.collect
                            (fun x -> x.Groups |> Option.defaultValue [||] |> Array.map (fun g -> g, lblMap.[x.Label]))
                        |> Seq.groupBy fst
