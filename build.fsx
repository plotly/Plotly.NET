...
module ProjectInfo =
    ...
    let gitHome =
        sprintf "%s/%s" "https://github.com" gitOwner
...
module BasicTasks =
    ...
    let copyBinaries =
        BuildTask.create "CopyBinaries" [ clean; build ] {
            let targets =
                !! "src/**/*.??proj" -- "src/**/*.shproj"
                |> Seq.map
                    (fun f ->
                        ((Path.getDirectory f) </> "bin" </> configuration,
...
module TestTasks =
    ...
    let runTests =
        BuildTask.create "RunTests" [ clean; build; copyBinaries ] {
            testProjects
            |> Seq.iter
                (fun t ->
                    let standardParams =
...
module VerificationTasks =
    ...
    let verifyDocs =
        BuildTask.create "VerifyDocs" [ clean; build; copyBinaries ] {
            let targets =
                !! "docs/**.fsx"
            ...
            let ignoredDiagnostics =
                Set.ofList [ 1182 ] // unused variable
...
module PackageTasks =
    ...
    let packPrerelease =
        BuildTask.create "PackPrerelease" [ setPrereleaseTag
                                            clean
                                            build
                                            runTests
                                            copyBinaries ] {
...
module DocumentationTasks =
    ...
    *)
    ...
    let buildDocs =
        BuildTask.create "BuildDocs" [ build; copyBinaries ] {
    ...
    let buildDocsPrerelease =
        BuildTask.create "BuildDocsPrerelease" [ setPrereleaseTag
                                                 build
                                                 copyBinaries ] {
    ...
    let watchDocs =
        BuildTask.create "WatchDocs" [ build; copyBinaries ] {
...
module ReleaseTasks =
    ...
    let createTag =
        BuildTask.create "CreateTag" [ clean.IfNeeded
                                       build.IfNeeded
    ...
    let createPrereleaseTag =
        BuildTask.create "CreatePrereleaseTag" [ setPrereleaseTag
                                                 clean.IfNeeded
                                                 build.IfNeeded
                                                 copyBinaries.IfNeeded
                                                 runTests.IfNeeded
                                                 packPrerelease.IfNeeded ] {
    ...
    let publishNuget =
        BuildTask.create "PublishNuget" [ clean
                                          build
                                          copyBinaries
                                          runTests
                                          pack ] {
    ...
    let publishNugetPrerelease =
        BuildTask.create "PublishNugetPrerelease" [ clean
                                                    build
                                                    copyBinaries
                                                    runTests
                                                    packPrerelease ] {
