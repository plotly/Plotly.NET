...
let ``Html layout tests`` =
    testList "SimpleTests.Simple tests" [
        testCase "Expecting plotly js script reference in generated html document" ( fun () ->
            ...
            |> substringIsInChart TestCharts.Chart2D.Point.``Point chart with axis labels and title`` toEmbeddedHTML 
        );
        testCase "Expecting data" ( fun () ->
            ...
            |> chartGeneratedContains TestCharts.Chart2D.Point.``Point chart with axis labels and title``
        );
        ...
        testCase "Expecting html tags in embedded page only" (fun () ->
            ...
            |> substringListIsInChart TestCharts.Chart2D.Point.``Point chart with axis labels and title`` toEmbeddedHTML
        );
        testCase "Expecting some html tags in both embedded and not embedded" (fun () ->
            ["<script type=\"text/javascript\">"; "</script>"]
            |> chartGeneratedContainsList TestCharts.Chart2D.Point.``Point chart with axis labels and title``
        );
...
let ``plotlyjs reference tests`` =
    testList "SimpleTests.plotlyjs reference" [
        testCase "full reference" (fun _ ->
            getFullPlotlyJS()
            |> substringIsInChart TestCharts.Chart2D.Point.``Point chart with full plotly.js reference`` toEmbeddedHTML
        )
        testCase "cdn reference" (fun _ ->
            $"""https://cdn.plot.ly/plotly-2.0.0.min"""
            |> substringIsInChart TestCharts.Chart2D.Point.``Point chart with plotly.js cdn reference`` toEmbeddedHTML 
        )
        testCase "require: Expecting cloudflare link" (fun () ->
            """https://cdnjs.cloudflare.com/ajax/libs/require.js"""
            |> chartGeneratedContains TestCharts.Chart2D.Point.``Point chart referencing plotly.js using require.js``
        );
